...
public class IntroBeatLineSpawner : MonoBehaviour
{
    ...
    private void Spawn()
    {
        ...
        _beatLine.GetComponent<IntroBeatLineMovementController>().Init(_events[_eventNumber]);
        GameObject _beatLineMirrored = ObjectPooler.Instance.SpawnFromPool(_poolTag);
        _beatLineMirrored.GetComponent<IntroBeatLineMovementController>().Init(_events[_eventNumber], -1f);
